		...
		init : function(options) {
			...
			return this.each(function(){
				...
				if ($data.length) {
					...
					var $ch_wrapper = $('<div>').addClass('ch_element').addClass(settings.wrapper_class).css({  'position':'relative', }).insertAfter($this);
					
					var $ch_normal = $('<div>').addClass(settings.normal_class).css({ 'width':w,  'z-index':settings.zindex }).appendTo($ch_wrapper);
					$this.clone().appendTo($ch_normal);
					$this.hide();

					var $ch_hover = $('<div>').addClass(settings.hover_class).css({ 'width':'100%', 'z-index':settings.zindex-1 }).appendTo($ch_wrapper);
